...
LORA_ALPHA = 32
LORA_DROPOUT = 0.05
LORA_TARGET_MODULES = None   # None = auto-detect common attention projection modules
# ═══════════════════════════════════════════════════════════════════════════════
...
def load_model_and_tokenizer(adapter_path: Optional[str] = None):
    ...
    if adapter_path and adapter_path != MODEL_DIR:
...
def get_lora_target_modules(model):
    ...
    common_names = {"q_proj", "k_proj", "v_proj", "o_proj"}
    found = sorted({
        name.split(".")[-1]
        for name, module in model.named_modules()
        if name.split(".")[-1] in common_names and isinstance(module, torch.nn.Linear)
    })
    if not found:
        raise ValueError(
            "Could not auto-detect LoRA target modules. "
            "Set LORA_TARGET_MODULES manually near the top of this file."
        )
...
def run_sft(model_path: str, data_path: str, output_path: str):
    """Fine-tune LoRA adapters on (prompt, revised_response) pairs."""
    ...
    print(f"Starting LoRA SFT: {model_path} -> {output_path}")
    ...
    if model_path != MODEL_DIR:
        print(f"Continuing from previous LoRA adapter: {model_path}")
    ...
    print(f"LoRA SFT complete. Adapter saved to {output_path}")

"""
ITERALIGN adapted for ibm-granite/granite-4.0-350m
...
MODEL_DIR = "./models/granite-4.0-350m"   # already downloaded here
MODEL_REPO = "ibm-granite/granite-4.0-350m"

# Check for HF_TOKEN for gated model access
HF_TOKEN = os.environ.get("HF_TOKEN", "")
if not HF_TOKEN:
    print("⚠️  WARNING: HF_TOKEN not set. This model may require authentication.")
    print("   1. Create a token at: https://huggingface.co/settings/tokens")
    print("   2. Accept any model license at: https://huggingface.co/ibm-granite/granite-4.0-350m")
    print("   3. Set it: export HF_TOKEN='your_token_here')")

# Helper to check if model is complete (has weights file)
def is_model_complete(model_dir: str) -> bool:
    """Check if model directory contains actual weights, not just metadata."""
    model_path = Path(model_dir)
    if not model_path.exists():
        return False
    # Look for weights files
    weights_found = (model_path / "model.safetensors").exists() or \
...
if not is_model_complete(MODEL_DIR):
    ...
    incomplete_path = Path(MODEL_DIR)
    if incomplete_path.exists():
        import shutil
        print(f"Removing incomplete model at {MODEL_DIR}...")
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to download model. Error: {e}\n"
            f"Please ensure:\n"
            f"  1. HF_TOKEN is set: export HF_TOKEN='your_token_here'\n"
            f"  2. Model access is available at https://huggingface.co/ibm-granite/granite-4.0-350m\n"
...
OUTPUT_DIR = f"./output_granite-4.0-350m_{DATASET}"
